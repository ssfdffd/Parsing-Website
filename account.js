...
if (token && name) {
  ...
  window.location.href = 'portal.html'; // Redirect to portal after GitHub login
}
...
if (existingToken) {
  fetch(`${WORKER_URL}/auth/verify`, { headers: { 'Authorization': `Bearer ${existingToken}` } })
    .then(r => {
      if (r.ok) window.location.href = 'portal.html'; // Auto-redirect if already logged in
      else {
        localStorage.removeItem('parsing_auth_token');
        sessionStorage.removeItem('parsing_auth_token');
      }
    })
    .catch(() => console.log('Offline or token invalid'));
}
